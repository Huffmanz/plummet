...
### Shop Drag Pipeline Juice

Polished the shop drag-and-drop pipeline across five files. `ShopOfferCard._start_drag()` now tweens `scale` → `Vector2.ZERO` and `modulate.a` → 0 over 0.12s (centered via `pivot_offset`) instead of instantly hiding, so the card visually shrinks while holding its layout space and sibling cards never shift. A `_drop_accepted` flag (set in `take_cursor_icon()`, cleared in `set_consumed()`) prevents `NOTIFICATION_DRAG_END` from flashing the card visible during the ~0.16s snap window; `set_drag_dim` also guards on this flag. Other offer cards dim to `Color(1,1,1,0.45)` on drag start and restore on drag end via `set_drag_dim()`. `ShopOfferDragIcon.snap_to(target_global_pos, callback, reduced_motion)` tweens the icon to the target slot center (0.16s, EASE_OUT/TRANS_BACK + shrink + fade), then `queue_free`s itself and fires the purchase callback — `shop_screen.gd` orchestrates this for all three offer kinds (modifier, piece type, relic) so purchase logic never runs before the snap lands. `ShopDropRingOverlay` spins at `ROT_SPEED * 1.4` and brightens to `Color(1.2, 1.2, 1.2)` while active. `ShopRelicSlot` pulses `modulate` `1.0 ↔ 1.15` over a 1s loop while drop-highlighted. `RelicManager._relics` changed from an append-only array to a fixed 4-slot array (`["","","",""]`); `add_relic(id, slot_idx)` places the relic at the requested slot index so drag-to-specific-slot is respected; `can_add_relic`, `relic_count`, and `has_relic` updated accordingly. All callers without a slot index fall back to the first empty slot.
